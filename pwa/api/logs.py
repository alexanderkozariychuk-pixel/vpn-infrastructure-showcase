...
@router.get("/api/logs")
async def get_all_logs(lines: int = Query(default=50, le=200), _: dict = Depends(require_admin)):
...
async def get_service_logs(service: str, lines: int = Query(default=50, le=200), _: dict = Depends(require_admin)):
